...
- 传输的数据不存在数据边界（Boundary）
   - 是指TCP将数据视为无结构的、连续的字节流，而不是分离的消息或数据包。这意味着，**当通过TCP发送数据时，不能预期接收方会以同样的数据块大小接收数据**。例如，如果发送了两个独立的数据块，接收方可能会一次性接收到这两个数据块，或者分多次接收。同样，如果一次发送了一个大的数据块，接收方可能会分多次接收这个数据块。
...
> 答：缓冲并不总是满的。如果读取速度比数据传入过来的速度慢，则缓冲可能被填满，但是这时也不会丢失数据，因为传输套接字此时会停止数据传输，所以面向连接的套接字不会发生数据丢失。面向连接的套接字会**根据接收端的状态**传输数据，如果传输出错还会提供**重传**服务。

面向连接的套接字必须**一一对应**，只能与另一个同样特性的套接字连接。可总结为：
...
面向消息的套接字比面向连接的套接字更具传输速度，但可能丢失。特点可总结为：
...
> 传递前两个参数即可创建所需套接字，所以大部分情况下可以向第三个参数传递0。  
