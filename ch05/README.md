...
二者都在循环调用 read 和 write 函数。实际上之前的回声客户端将 100% 接受自己传输的数据，只不过接受数据时的单位有些问题。扩展客户端代码回顾范围，下面是[echo_client](../ch04/echo_client.c)客户端的代码:
...
回声客户端可以提前知道接收数据的长度，这在大多数情况下是不可能的。那么此时无法预知接收数据长度时应该如何收发数据？此时需要的就是**应用层协议**的定义。在收发过程中定好规则（协议）以表示数据的边界，或者提前告知需要收发数据的大小。服务器端/客户端实现过程中逐步定义的规则集合就是应用层协议。

现在写一个小程序来体验应用层协议的定义过程。要求：

1. 服务器从客户端获得多个数字和运算符信息。
2. 服务器接收到数字后对齐进行加减乘运算，然后把结果传回客户端。

例：

1. 向服务器传递3,5,9的同时请求加法运算，服务器返回`3+5+9`的结果。
2. 请求做乘法运算，客户端会收到`3*5*9`的结果。
3. 如果向服务器传递4,3,2的同时要求做减法，则返回`4-3-2`的运算结果。
...
TCP 套接字的数据收发无边界。服务器即使调用 1 次 write 函数传输 40 字节的数据，客户端也有可能通过 4 次 read 函数调用每次读取 10 字节。但此处也有一些疑问，服务器一次性传输了 40 字节，而客户端竟然可以缓慢地分批接受。客户端接受 10 字节后，剩下的 30 字节在何处等候呢？
...
TCP 不会发生超过输入缓冲大小的数据传输。也就是说，根本不会发生这类问题，因为 TCP 会控制数据流。TCP 中有滑动窗口（Sliding Window）协议，用对话方式如下：
...
write 函数和 Windows 的 send 函数并不会在完成向对方主机的数据传输时返回，而是在**数据移到输出缓冲时**。但 TCP 会保证对输出缓冲数据的传输，所以说 write 函数在数据传输完成时返回。
...
- [shake1]套接字A：你好，套接字 B。我这里有数据给你，建立连接吧
...
套接字是全双工(Full-duplex)方式工作的。也就是说，它可以双向传递数据。因此，收发数据前要做一些准备。首先请求连接的主机 A 要给主机 B 传递以下信息：
...
上图表示了通过 SEQ 1301 数据包向主机 B 传递 100 字节数据。但中间发生了错误，主机 B 未收到，经过一段时间后，主机 A 仍然未收到对于 SEQ 1301 的 ACK 确认，因此试着重传该数据包。为了完成该数据包的重传，TCP 套接字启动计时器以等待 ACK 应答。若相应计时器发生超时（Time-out!）则重传。
...
图中数据包内的 FIN 表示断开连接。也就是说，双方各发送 1 次 FIN 消息后断开连接。此过程经历 4 个阶段，因此又称四次握手（Four-way handshaking）。SEQ 和 ACK 的含义与之前讲解的内容一致，省略。图中，向主机 A 传递了两次 ACK 5001，也许这里会有困惑。其实，第二次 FIN 数据包中的 ACK 5001 只是因为接收了 ACK 消息后未接收数据而重传的。
...
1. **请说明 TCP 套接字连接设置的三次握手过程。尤其是 3 次数据交换过程每次收发的数据内容。**
   ...
   答：
   1. SYN：主机1发送一个 SYN（同步）包给主机2，请求建立连接。这个包中包含一个随机生成的序列号 A。
   2. SYN-ACK：主机2收到 SYN 包后，返回一个 SYN-ACK（同步确认）包给主机1。这个包中包含一个随机生成的序列号 B，以及一个确认号 ACK，这个确认号的值是 A+1。
   3. ACK：主机1收到 SYN-ACK 包后，返回一个 ACK（确认）包给主机2。这个包中的确认号的值是 B+1。


2. **TCP 是可靠的数据传输协议，但在通过网络通信的过程中可能丢失数据。请通过 ACK 和 SEQ 说明 TCP 通过何种机制保证丢失数据的可靠传输。**

   答：
   - 序列号（SEQ）：TCP 为每个发送的字节分配一个唯一的序列号。这个序列号在整个 TCP 连接期间是持续增长的。当发送一个数据包时，TCP 会在数据包的头部添加当前的序列号。这样，接收方就可以知道每个数据包的位置，即使数据包到达的顺序与发送的顺序不一致，接收方也可以根据序列号将它们重新排序。

   - 确认（ACK）：当接收方收到一个数据包后，它会发送一个 ACK 包给发送方，确认它已经收到了这个数据包。ACK 包中包含一个确认号，这个**确认号的值是接收方期望接收的下一个字节的序列号(ACK = SEQ + 传输字节数 + 1)**。如果发送方没有在一定时间内收到 ACK，它会认为数据包已经丢失，并重新发送这个数据包。
...
4. **对方主机的输入缓冲剩余 50 字节空间时，若本主机通过 write 函数请求传输 70 字节，请问 TCP 如何处理这种情况？**
   ...
   答：
         ...
         TCP使用滑动窗口机制来控制数据的发送和接收。滑动窗口的大小表示的是接收方当前能够接收的数据量。接收方会在 ACK（确认）包中告诉发送方它的窗口大小。

         在这个例子中，如果对方主机的输入缓冲只剩下 50 字节的空间，那么它会在 ACK 包中告诉本主机，它的窗口大小是 50 字节。当本主机收到这个 ACK 包后，它会知道它只能发送 50 字节的数据。

         如果本主机尝试通过 write 函数发送 70 字节的数据，那么 TCP 会将这 70 字节的数据分成两部分。首先，它会发送 50 字节的数据，然后等待接收到新的 ACK 包，这个 ACK 包会告诉它新的窗口大小。当接收到新的 ACK 包后，TCP 会发送剩下的 20 字节的数据。
