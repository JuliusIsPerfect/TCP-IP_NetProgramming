...
1. **下列关于 MSG_OOB 可选项的说法错误的是**？
   ...
   1. MSG_OOB 指传输 Out-of-band 数据，是通过其他路径高速传输数据：在 TCP/IP 中，所有数据，包括标记为 OOB 的数据，都通过相同的路径和速度传输。OOB 数据的特殊之处在于，接收端的 TCP 栈会对其进行特殊处理，使得接收应用程序可以选择立即处理这些数据，而不是按照正常的数据流顺序。
   2. MSG_OOB 指通过其他路径高速传输数据，因此 TCP 中设置该选项的数据先到达对方主机：同上。
   3. 设置 MSG_OOB 使数据先到达对方主机后，以普通数据的形式和顺序读取。也就是说，只是提高了传输速度，接收方无法识别这一点：MSG_OOB并不会提高数据的传输速度，也不会改变数据的到达顺序。接收方可以选择在接收到OOB数据时立即处理，而不是等待整个数据流被读取完毕。
...
2. **利用 readv & writev 函数收发数据有何优点？分别从函数调用次数和 I/O 缓冲的角度给出说明**。
   ...
   答：
   它们允许在单个系统调用中进行多个非连续缓冲区的读写操作。这有两个主要的优点：

   1. 减少函数调用次数：如果你有多个非连续的缓冲区需要读写，使用readv和writev可以在一个系统调用中完成，而不是为每个缓冲区进行单独的read或write调用。这可以减少系统调用的开销，特别是在大量数据传输的情况下。

   2. 更有效的I/O缓冲：readv和writev允许数据直接从源到多个目标缓冲区，或者从多个源缓冲区到目标，而无需在用户空间中进行额外的数据复制。这可以减少CPU的使用，提高I/O操作的效率。

3. **通过 recv 函数验证输入缓冲中是否存在数据时（确认后立即返回时），如何设置 recv 函数最后一个参数中的可选项？分别说明各可选项的含义**。

   答：

   MSG_PEEK: 这个选项会让 recv 函数查看即将接收的数据，但不会从输入缓冲中移除这些数据。这意味着下一次调用recv函数时，同样的数据仍然会被接收。

   MSG_DONTWAIT: 这个选项会让 recv 函数在数据未准备好时立即返回，而不是阻塞。
