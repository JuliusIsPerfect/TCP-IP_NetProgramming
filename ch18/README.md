...
2. **为何线程的上下文切换速度相对更快？线程间数据交换为何不需要类似 IPC 特别技术**。
   ...
   答：线程上下文切换过程不需要切换数据区和堆。线程是在同一进程空间内运行的，它们共享相同的地址空间。因此线程之间可以直接访问相同的内存变量和数据结构，可以利用数据区和堆交换数据。
...
4. **下面关于临界区的说法错误的是**？
   ...
   2. 线程安全的函数中不存在临界区，即便多个线程同时调用也不会发生问题：线程安全的函数中同样可能存在临界区。
   3. 1 个临界区只能由 1 个代码块，而非多个代码块构成。换言之，线程 A 执行的代码块 A 和线程 B 执行的代码块 B 之间绝对不会构成临界区：线程A和线程B可以有各自的代码块，如果这些代码块访问相同的共享资源，那么它们就构成了临界区。
   4. 临界区由访问全局变量的代码构成。其他变量中不会发生问题：任何访问共享资源（如全局变量、静态变量、堆上的对象、共享内存等）的代码都可能构成临界区。
...
5. **下列关于线程同步的说法错误的是**？
   ...
   3. **互斥量和信号量是典型的同步技术**。
   4. 线程同步是代替进程 IPC 的技术：线程同步用于在同一进程的多个线程之间同步访问共享资源，而 IPC 是用于不同进程之间的数据交换和同步。

6. **请说明完全销毁 Linux 线程的 2 种办法**。

   答：
   1. 调用 pthread_join 函数：阻塞当前线程，直到指定的线程结束。当线程结束时，pthread_join会回收线程的资源并获取线程的退出状态。这是一种显式的资源回收方式，需要程序员明确地在代码中调用。如果不调用pthread_join（或pthread_detach），那么即使线程已经结束，其资源（如线程的栈）也不会被完全回收，这会导致资源泄漏。
   2. 调用 pthread_detach 函数：将线程设置为分离状态。当一个分离状态的线程结束时，其资源会自动被回收。这是一种隐式的资源回收方式，适用于那些不需要获取退出状态，也不需要明确知道何时结束的线程。

   对于同一个线程，只能调用pthread_join或pthread_detach中的一个。如果一个线程已经被分离，那么再试图调用pthread_join会失败，反之亦然。
